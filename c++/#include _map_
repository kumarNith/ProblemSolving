...
int main() {
    ...
    BST bst;
    bst.insertLeft(root->left, 2);
    bst.insertLeft(root->left->left, 1);
    bst.insertLeft(root->right, 1);
    bst.insertLeft(root->right->right, 2);
    bst.insertLeft(root->right->left, 1);
    cout<<"No of paths exit :"<<bst.findNuberofPath(root, 5)<<endl;
